...
void SSC1::Handler()
{
  input_l = (int32_t)(SSC_RHR);  // SSC data input
  ...
  #if defined(PASS_THROUGH) || defined(COMP)
  ...
  SSC_THR = sample;  // SSC data output
}
...
void measure_system()
{
    ...
    // coefficients for straight line
    #ifdef OPT_LINEARISATION
    ua1 = ((float)adc_volt_anode[0])/1000000;
    ug1 = (((float)dac_volt[0])/1000000);
    #endif /* OPT_LINEARISATION */
...
/* ************************************************************************* */
/*                                                                           */
/* convert_sample_to_voltage                                                 */
/*                                                                           */
/* ************************************************************************* */
int32_t convert_sample_to_voltage(int32_t sample)
{
  int max_out = 3300000; // input voltage range
...
/* ************************************************************************* */
/*                                                                           */
/* convert_voltage_to_sample                                                 */
/*                                                                           */
/* ************************************************************************* */
int32_t convert_voltage_to_sample(int32_t sample)
{
  /* maximum voltage output is 1V(RMS), so 2.82Vpp */
